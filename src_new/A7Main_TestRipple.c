...
LRESULT CALLBACK A7MainWinProc ( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam ) {
    ...
    switch ( uMsg ) {
        ...
        case WM_LBUTTONDOWN: {
            A7Node_AddFirstChild ( ( S7Node* ) p, ( S7Node* ) A7Node_New_Ripple ( NULL, NULL, LOWORD ( lParam ), HIWORD ( lParam ) ) );
            InvalidateRect ( hWnd, NULL, FALSE );
            return 0;
        }
        case WM_LBUTTONUP: {
            for ( S7Node *pC = ( ( S7Node* ) p ) -> pChildFirst; pC != NULL; pC = pC -> pSiblingNext ) {
                switch ( ( ( S7Base* ) pC ) -> iType ) {
                    case DT7_NODE_RIPPLE:
                        if ( ( ( S7Node_Ripple* ) pC ) -> iClockEnd == 0 ) {
                            ( ( S7Node_Ripple* ) pC ) -> iClockEnd = clock ();
                        }
                        break;
                }
            }
